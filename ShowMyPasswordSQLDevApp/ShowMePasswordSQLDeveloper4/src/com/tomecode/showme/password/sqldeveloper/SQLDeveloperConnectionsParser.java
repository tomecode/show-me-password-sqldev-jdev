...
public final class SQLDeveloperConnectionsParser {
    ...
    public static final List<DBLogin> loadDbConfig() {
        ...
        try {
            path =
                (String) DefaultContextWrapper.getInstance().getDatabaseContext().getEnvironment().get("java.naming.provider.url");
        } catch (Exception e) {
                ...
                for (int ix = 0; ix <= nlRefAddresses.getLength() - 1; ix++) {
                    ...
                    try {
                        ...
                        if ("RefAddresses".equals(eRefAddress.getNodeName())) {
                            ...
                            String serviceName =
                                parseValue(eRefAddress.getElementsByTagName("StringRefAddr"), "serviceName");

                            if (hostname == null || hostname.trim().length() == 0) {
                                String customUrl =
                                    parseValue(eRefAddress.getElementsByTagName("StringRefAddr"), "customUrl");
